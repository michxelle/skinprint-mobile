...
class PersonalizedComparisonPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('My Skinprint')),
      body: SafeArea(
        top: false,
        child: ListView(
          ...
          children: [
            ...
            _ComparisonOverview(comparison: comparison),

            const SizedBox(height: 34),

            const Divider(),

            if (comparison.didntWorkOnlyMatches.isNotEmpty) ...[
            ...
            if (comparison.workedOnlyMatches.isNotEmpty) ...[
              const SizedBox(height: 30),
            ...
            if (comparison.mixedMatches.isNotEmpty) ...[
              const SizedBox(height: 30),

              Text('Mixed history', style: AppTextStyles.sectionTitle()),
            ...
            Text('About this comparison', style: AppTextStyles.sectionTitle()),
            ...
            if (comparison.ignoredNeutralProducts > 0) ...[
              const SizedBox(height: 10),
...
class _ComparisonOverview extends StatelessWidget {
  ...
  const _ComparisonOverview({required this.comparison});
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (match.workedProductNames.isNotEmpty)
            _HistoryEvidence(
              label: 'Worked for you',
              productNames: match.workedProductNames,
              color: AppColors.good,
            ),

          if (match.workedProductNames.isNotEmpty &&
              match.didntWorkProductNames.isNotEmpty)
            const SizedBox(height: 10),

          if (match.didntWorkProductNames.isNotEmpty)
            _HistoryEvidence(
              label: 'Didn’t work for you',
              productNames: match.didntWorkProductNames,
